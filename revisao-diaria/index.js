...
1. Use list_chats ou search_contacts pra encontrar a conversa chamada "Caixa de Entrada" (é uma conversa onde a própria Jéssica envia items pra si mesma)
2. Use list_messages nessa conversa pra buscar as mensagens mais recentes (~30-50 últimas)
3. Identifique a última mensagem com texto EXATAMENTE "OK" enviada pela PRÓPRIA Jéssica (não por outra pessoa). Essa "OK" é o marcador de "até aqui já processei"
4. Processe APENAS as mensagens que estão ABAIXO (mais recentes) desse último OK:
...
5. Após processar TODOS os items, envie a mensagem "OK" via send_message pra essa mesma conversa — isso marca até onde processou
6. Depois do OK, envie cada item não-tarefa de volta na mesma conversa via send_message — isso deixa visível pra Jéssica decidir o que fazer com eles

Demais conversas não arquivadas:
1. Use list_chats pra ver as conversas não arquivadas
2. Identifique as que tiveram atualização recente (últimas 24h, por exemplo)
3. Pra cada conversa com atualização: use list_messages pra ver as mensagens novas, identifique items de ação, crie tarefas no Notion (sem prazo a não ser que mencionado explicitamente)
...
- NUNCA mande mensagens em conversas que não sejam a "Caixa de Entrada".
